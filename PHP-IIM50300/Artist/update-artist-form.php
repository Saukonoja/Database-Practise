...
include($artistUpdateFormQuery);
$result = $conn->query("$sql");

if ($row = $result->fetch_assoc()){
  ?>
    <form action='<?php echo $artists ?>'><button id='btnBack' class='buttons'><i id="backIcon" class="fa fa-arrow-left fa-lg"></i></button></form>
    <div id="updateForm">
      <h1>Update artist</h1>
      <form method='post' action='<?php echo $updateArtist ?>'>
        ...
        <input type='submit' name='action' value='Delete artist' class="buttons" id="deleteButton" onclick="javascript: return confirm('Delete artist <?php echo $row['nimi'] ?> ?')"><br>
      </form>
    </div>  
 <?php   
}
